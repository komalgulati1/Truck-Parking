...
COST_FULL_SERVICE_MAX = 13e6  # $13 million for full-service facilities (maximum)
...
def load_data():
    ...
    print(f"Loaded {len(candidates_df)} candidate locations from composite scores CSV")
    ...
    print(f"Loaded {len(existing_facilities)} existing facilities")
...
def calculate_development_costs(candidates_gdf):
    ...
    # UPDATED COST MODEL CONSTANTS - Optimized for target ranges
    SITE_PREP_NO_SERVICE = 200000  # $200,000 for basic site preparation (unchanged)
    COST_PER_SPACE_NO_SERVICE = 10000  # $10,000 per space for no-service (unchanged)
    
    # NEW FULL-SERVICE COST PARAMETERS - Designed to hit $11M-$13M range
    SITE_PREP_FULL_SERVICE = 7000000   # $7.0 million for full-service site preparation  
    COST_PER_SPACE_FULL_SERVICE = 67000  # $67,000 per space for full-service
...
def create_appendix_tables(no_service_selection, full_service_selection):
    ...
    with detailed county information from finalized_shortlisted_candidates.shp
    """
    ...
    # Load finalized_shortlisted_candidates.shp to get ComplexCom and CNTY_NM values
    try:
        candidates_path = os.path.join(script_dir, "../datasets/finalized_shortlisted_candidates.shp")
        finalized_shortlisted_candidates_gdf = gpd.read_file(candidates_path)
        print(f"Loaded finalized_shortlisted_candidates.shp with {len(finalized_shortlisted_candidates_gdf)} features")
        
        # Print column names to verify CNTY_NM column exists
        print("Columns in finalized_shortlisted_candidates.shp:", finalized_shortlisted_candidates_gdf.columns.tolist())
        ...
        for col in possible_county_columns:
            if col in finalized_shortlisted_candidates_gdf.columns:
                county_column = col
                print(f"Found county column: {col}")
                # Show sample values
                print(f"Sample county values from {col}:")
                print(finalized_shortlisted_candidates_gdf[col].head())
                break
        
        if county_column is None:
            print("WARNING: No county column found in finalized_shortlisted_candidates.shp!")
            # Look for any string columns that might contain county names
            string_cols = [col for col in finalized_shortlisted_candidates_gdf.columns if 
                         finalized_shortlisted_candidates_gdf[col].dtype == 'object' and 
        ...
        if 'FID' not in finalized_shortlisted_candidates_gdf.columns:
            # Try alternative ID columns
            potential_id_fields = ['OBJECTID', 'ID', 'GID', 'OID']
            for field in potential_id_fields:
                if field in finalized_shortlisted_candidates_gdf.columns:
                    finalized_shortlisted_candidates_gdf['FID'] = finalized_shortlisted_candidates_gdf[field]
                    break
            else:
                # If no ID column found, use index
                finalized_shortlisted_candidates_gdf['FID'] = finalized_shortlisted_candidates_gdf.index
        
        # Convert FID to string for consistent joining
        finalized_shortlisted_candidates_gdf['FID'] = finalized_shortlisted_candidates_gdf['FID'].astype(str)
    except Exception as e:
        print(f"Error loading finalized_shortlisted_candidates.shp: {e}")
        return
    ...
    def create_appendix_for_scenario(selection_df, scenario_name):
        ...
        # Merge with finalized_shortlisted_candidates_gdf to get both ComplexCom and county information
        if county_column:
            # Use both ComplexCom and county column
            appendix_df = unique_facilities.merge(
                finalized_shortlisted_candidates_gdf[['FID', 'ComplexCom', county_column]], 
                on='FID', 
                how='left'
            )
            ...
            appendix_df = unique_facilities.merge(
                finalized_shortlisted_candidates_gdf[['FID', 'ComplexCom']], 
                on='FID', 
                how='left'
            )
